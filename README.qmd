...
in a DataFrame.
It works with DataFrame objects from two popular libraries: `pandas` and `polars`.
Importantly, neither library needs to be installed.
...
Note that the abstract classes can be used as stand-ins for the real thing
in `issubclass()` and `isinstance`.
...
Python has a built-in function implementing this called [`functools.singledispatch`](https://docs.python.org/3/library/functools.html#functools.singledispatch).
...
Under the hood, `AbstractBackend` behaves similarly to python's builtin [`abc.ABC` class](https://docs.python.org/3/library/abc.html#abc.ABC).
